...
vi.mock('@/components/DatePicker', () => ({
  ...
  default: ({ name, value, onChange, style }: {
    ...
    <div>
      ...
      <button type="button" onClick={() => onChange(new Date())} style={style} data-testid="date-picker-btn">
...
describe('ContactForm', () => {
  ...
  it('shows invalid email error when email format is wrong', async () => {
    render(<ContactForm />)
    fireEvent.change(screen.getByPlaceholderText('Email'), { target: { value: 'emailinvalide' } })

    const submitBtn = screen.getByRole('button', { name: /Envoyer/i })
    fireEvent.click(submitBtn)

    await waitFor(() => {
      expect(screen.getByText("Le format de votre adresse email n'est pas valide")).toBeInTheDocument()
